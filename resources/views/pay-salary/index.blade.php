                ...
                <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
                    ...
                    <div>
                        <a href="{{ route('pay-salary.create') }}" class="btn btn-info add-list mr-1">
                            <x-heroicon-o-plus class="w-5 h-5 mr-1 inline" /> Pay Custom
                        </a>
                        <a href="{{ route('pay-salary.payAllView') }}" class="btn btn-success add-list mr-1">
                            <x-heroicon-o-banknotes class="w-5 h-5 mr-1 inline" /> Pay All
                        </a>
                        <a href="{{ route('pay-salary.payHistory') }}" class="btn btn-warning add-list">
                            <x-heroicon-o-clock class="w-5 h-5 mr-1 inline" /> Pay History
                        </a>
                    </div>
                </div>
            </div>
                    ...
                    <table class="table mb-0">
                        ...
                        <tbody class="ligth-body">
                            @forelse ($advanceSalaries as $advance)
                                <tr>
                                    <td>{{ ($advanceSalaries->currentPage() - 1) * $advanceSalaries->perPage() + $loop->iteration }}</td>
                                    <td>
                                    ...
                                    <td>Rp. {{ number_format($advance->employee->salary, 0, ',', '.') }}</td>
                                    <td>
                                        <span class="badge badge-warning">Rp. {{ number_format($advance->advance_salary, 0, ',', '.') }}</span>
                                    </td>
                                    <td>
                                        <span class="text-success font-weight-bold">
                                            Rp. {{ number_format($advance->employee->salary - $advance->advance_salary, 0, ',', '.') }}
                                        </span>
                                    </td>
                                    <td>
