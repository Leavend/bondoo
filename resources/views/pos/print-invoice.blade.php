            ...
            <div class="sc-card shadow-sm border-0 rounded-lg">
                <div class="card-body p-0">
                    ...
                    <div class="p-5">
                        ...
                        <div class="row mb-5">
                            <div class="col-6">
                                <h3 class="font-weight-bold text-dark mb-1">bondoo</h3>
                                <p class="text-muted mb-0">Bontang, East Kalimantan</p>
                                ...
                                <tbody>
                                    @foreach($orderDetails as $item)
                                        <tr>
                                            ...
                                            <td class="border-bottom-0 text-right py-3">Rp. {{ number_format($item->unit_price, 0, ',', '.') }}</td>
                                            <td class="border-bottom-0 text-right py-3 pr-4 font-weight-bold">Rp. {{ number_format($item->total, 0, ',', '.') }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                            ...
                            <div class="col-md-5 ml-auto">
                                <table class="table table-sm table-borderless">
                                    <tr>
                                        ...
                                        <td class="text-right font-weight-bold">Rp. {{ number_format($order->sub_total, 0, ',', '.') }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-muted">Tax (VAT)</td>
                                        <td class="text-right font-weight-bold">Rp. {{ number_format($order->vat, 0, ',', '.') }}</td>
                                    </tr>
                                    <tr class="border-top">
                                        <td class="text-dark font-weight-bold pt-3 h5">Total</td>
                                        <td class="text-primary font-weight-bold text-right pt-3 h5">Rp. {{ number_format($order->total, 0, ',', '.') }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-muted">Paid</td>
                                        <td class="text-success text-right font-weight-bold">Rp. {{ number_format($order->pay_amount, 0, ',', '.') }}</td>
                                    </tr>
                                     <tr>
                                        <td class="text-muted">Change</td>
                                        <td class="text-dark text-right font-weight-bold">Rp. {{ number_format($order->due_amount < 0 ? abs($order->due_amount) : 0, 0, ',', '.') }}</td>
                                    </tr>
                                </table>
                            </div>
                        </div>
