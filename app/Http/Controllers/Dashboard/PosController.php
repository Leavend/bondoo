...
class PosController extends Controller
{
    ...
    public function addCart(Request $request)
    {
        ...
        Cart::add([
            ...
            'options' => [
                'unit_type' => 'primary',
                'primary_unit' => $product->primary_unit ?? 'pcs',
                'secondary_unit' => $product->secondary_unit ?? '',
                'conversion_rate' => $product->conversion_rate ?? 1,
                'wholesale_price' => $product->wholesale_price ?? null,
                'wholesale_minimum_qty' => $product->wholesale_minimum_qty ?? 0
            ]
        ]);
    ...
    /**
     * Update item quantity and unit type in the cart.
     */
    public function updateCart(Request $request, string $rowId)
    {
        $validatedData = $request->validate([
            'qty' => 'required|numeric',
            'unit_type' => 'nullable|in:primary,secondary',
        ]);
        ...
        $unitType = $request->input('unit_type', $cartItem->options->unit_type ?? 'primary');

        // Determine price
        $price = $product->selling_price;
        if ($unitType === 'secondary' && $product->wholesale_price) {
            $price = $product->wholesale_price;
        } else {
            // Apply wholesale price if wholesale quantity threshold is met
            if ($product->wholesale_price && $product->wholesale_minimum_qty > 0 && $qty >= $product->wholesale_minimum_qty) {
                $price = $product->wholesale_price;
            }
        }
