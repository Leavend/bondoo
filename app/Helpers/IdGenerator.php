...
class IdGenerator
{
    private function getFieldType($table, $field)
    {
        ...
        if ($driver === 'sqlite') {
            // For SQLite, query column type using PRAGMA
            $rows = DB::select("PRAGMA table_info($table)");
            $fieldType = null;
            $fieldLength = 255; // SQLite doesn't strictly limit varchar length
    ...
    public static function generate($configArr)
    {
        ...
        if ($resetOnPrefixChange) {
            $whereClause = "WHERE " . $field . " LIKE '" . $prefix . "%'";
            if (array_key_exists('where', $configArr) && count($configArr['where'])) {
                $conditions = [];
                foreach ($configArr['where'] as $row) {
                    $conditions[] = $row[0] . "=" . $row[1];
                }
            ...
            $maxQuery = sprintf("SELECT MAX(%s) AS maxid FROM %s %s", $field, $table, $whereClause);
        } else {
            $maxQuery = sprintf("SELECT MAX(%s) AS maxid FROM %s %s", $field, $table, $whereString);
        }

        $queryResult = DB::select(trim($maxQuery));
